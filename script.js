...
function randomEnemy() {
    ...
    if (r < 0.7) {
        ...
        return {
            type: 'letter',
            letter: MAINNET_LETTERS[Math.floor(Math.random() * MAINNET_LETTERS.length)],
            x: Math.random() * (GAME_WIDTH - 40),
            y: Math.random() * -600 - 40,
            width: 44,
            height: 44
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter') {
        ...
        if (!gameStarted && walletConnected) {
            ...
            showBonusMessage('Connect wallet first');
        }
    }
});
...
startButton.addEventListener('click', () => {
    if (walletConnected) startGame();
    else showBonusMessage('Connect wallet first');
});
...
async function pauseGameForFollow(key) {
    ...
    btn.onclick = () => {
        grantFollowBonus(key);
        btn.onclick = null;
    };
}

function grantFollowBonus(key) {
    if (key === 'pharos' && pharosUnlocked) {
        score += 3;
    } else if (key === 'blocksense' && blocksenseUnlocked) {
        score += 5;
    }
    updateScore();
...
function showBonusMessage(message) {
    ...
    setTimeout(() => {
        if (bonusMessage.textContent === message) bonusMessage.textContent = '';
    }, 3000);
}
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
            if (isColliding(bullets[i], enemies[j])) {
                ...
                if (hit.type === 'pharos') {
                    ...
                    score += 1;
                    showBonusMessage(`FIRE RATE UP! (${shootInterval}ms)`);
                }
                updateScore();
                break;
            }
...
function drawGame() {
    ...
    bullets.forEach(b => {
        if (blocksenseImage.complete) {
            ctx.drawImage(blocksenseImage, b.x, b.y, b.width, b.height);
        } else {
            ctx.fillStyle = '#f00';
    ...
    enemies.forEach(e => {
        if (e.type === 'pharos') {
            if (pharosImage.complete) {
                ctx.drawImage(pharosImage, e.x, e.y, e.width, e.height);
            } else {
                ctx.fillStyle = '#00f';
